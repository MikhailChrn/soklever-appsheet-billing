...
**Текущий рабочий бэкап:** v16 (13.05.2026).
...
| 13.05.2026 | [briefing-pre09](briefings/2026-05-13_briefing-pre09.md) | Sonnet | v15, 1.000573 | В работе. Взаимозачёты: при сохранении payment с `payment_type = "Взаимозачёт"` — автоматический LINKTOFORM на форму expense с предзаполнением. |
...
| 13.05.2026 | [report-15](reports/2026-05-13_soklever-appsheet-report-15.md) | (без брифа — диагностическая сессия SubGeneral #15) | 1.000569 → 1.000573 | Сдан. Устранён рассинхрон `current_period`: переведён из VC в физическую колонку, action `settings__update_trigger` расширен для пересчёта перед запуском бота. Тестовый прогон: 8 charges за 202606 создано корректно. Очищено 16 строк-мусора. Косметический хвост: сообщение бота пишет «Период 202606 закрыт» вместо 202605 (опережает из-за порядка update→ForEach→message). |
