...
**Эталон схемы:** **v1.1** (15.05.2026) — файл [`reference-data/2026-05-15 soklever-schema-v1.1.xlsx`](reference-data/2026-05-15%20soklever-schema-v1.1.xlsx). Манифест: [`reference-data/schema-v1.1-manifest.md`](reference-data/schema-v1.1-manifest.md). 13 листов, 0 формул. Собран из выгрузки рабочей книги от 15.05 удалением листов-рудиментов (`dashboard`, `debts`). Используется для раскатки на чистом аккаунте и DR.

**Состав листов:** 13 на момент 15.05.2026 (эталон v1.1):
...
**Удалены из эталона** (присутствуют в рабочей книге Sheets, но не подключены к AppSheet и не входят в v1.1):
- `dashboard` — legacy-отчёт на Sheets-формулах эпохи до AppSheet (7784 формулы, многие с `#REF!`). См. §6.2 п.4.
- `debts` — мёртвая таблица, по Р-5 не используется (динамический долг через VC). См. §2.14, §6.2 п.3.

Эти листы остаются в живой рабочей книге до отдельного решения об удалении из неё. В эталон v1.1 не входят.

### §1.2. Приложение

**AppSheet:** `soklever-root-billing v.1.0`. Версия на 15.05.2026 — **1.000593** (после #09: эталон v1.1, удаление `opening_balances` из донора, кнопка «летний» прикрыта на view-фильтрах через `CONTEXT("View") = "скрипты"`, `tariffs` → Updates only).
...
**Мёртвая** таблица в AppSheet. По Р-5 не используется. К удалению из живой рабочей книги (см. §6). **Не входит в эталон v1.1** — удалена при сборке 15.05.2026.

### §2.15. Листы-рудименты в Sheets

`dashboard`, `debts` — рудименты эпохи до AppSheet. Не подключены к приложению. **Не входят в эталон v1.1** — удалены при сборке 15.05.2026. В живой рабочей книге остаются до отдельного решения об удалении.
...
1. **`opening_balances`** — **✅ удалена (15.05.2026, в той же сессии #18 после раскатки v1.1).** Стартовые долги вносятся ручным начислением в открытом периоде. Порядок исполнения: (1) убраны `- SUM(SELECT(opening_balances[amount], ...))` из `students[debt]` и `students[overdue_debt]` (v.1.000587 → v.1.000588), (2) удалена VC `Related opening_balance` из `students` (v.1.000589), (3) удалена таблица из AppSheet (v.1.000590), (4) удалён лист `opening_balances` из Sheets, (5) Regenerate Schema (v.1.000591). Из эталона v1.1 — будет удалена при следующей сборке.
...
7. **`CONTEXT("View")`** возвращает имя текущего view. Использовать в Show_If, чтобы action не лез на чужие view-фильтры через общий slice. Канонический паттерн в проекте: `AND( LOOKUP(USEREMAIL(), "users", "email", "role") = "ADMIN", CONTEXT("View") = "скрипты" )` — actions `settings__close_period_trigger`, `settings__apply_summer_tariffs` (последний поправлен 15.05.2026, до этого светился на view-фильтрах).
...
- **0.6** (15.05.2026) — фиксация эталона схемы **v1.1**. §1.1 переписан: добавлен блок «Эталон схемы» со ссылкой на файл `2026-05-15 soklever-schema-v1.1.xlsx` и манифест `schema-v1.1-manifest.md`; состав 13 листов привязан к эталону v1.1; листы `dashboard` и `debts` явно помечены как «вне эталона». §2.14 и §2.15 — приписка «не входит в эталон v1.1». Сборка выполнена из выгрузки `2026-05-15 soklever-google-ancestor-v6 (main).xlsx` удалением двух листов-рудиментов; проверки: формул в эталоне 0, именованных диапазонов нет, скрытых листов нет, защит нет.
- **0.7** (15.05.2026, вечер) — после раскатки v1.1 и тестирования. §1.2 — версия `soklever-root-billing v.1.0` обновлена до 1.000593. §6.2 п.1 (`opening_balances`) переведён в статус «удалена», расписан порядок исполнения (5 шагов, v.1.000587 → v.1.000591). §8 п.7 (`CONTEXT("View")`) дополнен каноническим паттерном проекта и историей фикса `settings__apply_summer_tariffs` (v.1.000592). `tariffs` приведён к Updates only (v.1.000593). Эталон v1.1 — будет пересобран без `opening_balances` следующим шагом.
